...
int main(int argc, char **argv) 
{
  ...
  MPI_File_open(MPI_COMM_WORLD, argv[1], MPI_MODE_RDONLY, Lustre_info, &input_NGSC);
  MPI_File_open(MPI_COMM_WORLD, argv[2], MPI_MODE_CREATE | MPI_MODE_WRONLY, Lustre_info, &output_FASTQ);
  ...
  for (std::vector<SubBlock>::iterator it = p_subblocks.begin(); it != p_subblocks.end(); ++it)
  {
    ...
    // Must wait in case write_buffer is still being processed
